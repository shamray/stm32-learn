...
pub fn gpio_init_mode(port: u32, pin: u32, mode: Mode) {
    let gpio_mode_reg_addr = reg::addr(port, 0x00);
    let mode_value = match mode {
        Mode::Input => 0x0,
        Mode::Output(_) => 0x1,
    };
    reg::set_bits(gpio_mode_reg_addr, pin * 2, mode_value, 0x3);

    match mode {
        Mode::Output(t) => {
            let gpio_output_type_reg_addr = reg::addr(port, 0x04);
            let output_type_value = match t {
                OutputType::PushPull => 0x0,
                OutputType::OpenDrain => 0x1,
            };
            reg::set_bits(gpio_output_type_reg_addr, pin, output_type_value, 0x1);
        }
        Mode::Input => {
...
pub fn gpio_enable_clock(port: u32) {
    ...
    match port {
        GPIOA_BASE => reg::set_bit(rcc_ahb1enr_addr, 0, true),
        GPIOB_BASE => reg::set_bit(rcc_ahb1enr_addr, 1, true),
        GPIOC_BASE => reg::set_bit(rcc_ahb1enr_addr, 2, true),
        GPIOD_BASE => reg::set_bit(rcc_ahb1enr_addr, 2, true),
        GPIOE_BASE => reg::set_bit(rcc_ahb1enr_addr, 2, true),
        GPIOH_BASE => reg::set_bit(rcc_ahb1enr_addr, 2, true),
        _ => {}
    }
...
pub fn gpio_get_pin(port: u32, pin: u32) -> bool {
    let gpio_idr_addr = reg::addr(port, 0x10);
    reg::get_bit(gpio_idr_addr, pin)
}
